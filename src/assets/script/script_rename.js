$(document).ready(function() {
  ...
  var distress = [
    [8, 11, true, true, true, true],
    [27, 29, true, true, true, true],
    [46, 48, true, true, true, true],
    [57, 58.9, true, true, true, true],
    [69, 71, true, true, true, true],
    [82, 83, true, true, true, true],
    [83, 84, true, true, true, true],
    [92, 94.5, true, true, true, false],
    [97, 98, true, true, true, false],
    [104, 108.9, true, true, false, true]
  ];
  function distressObserver(video, audio) {
    $(".observer-text")
      .html("Distress Observed")
      .addClass("red-color");
    $(".observer-bar").addClass("red-backgroundcolor");
  ...
  function onTimeUpdate(event) {
    ...
    for (var index = 0; index <= distress.length; index++) {
      ...
      if (currentTime >= element[0] && element[2] == false) {
        //start the blink;
        distressObserver(element[4], element[5]);
		element[2] == true
		element[3] == false
      }
      if (currentTime >= element[1] && element[2] == true && element[3] == false) {
        //stop the blink
        element[3] = true;
		element[2] == false
    ...
    /*
    distress.forEach(function(element) {
      if (currentTime >= element[0] && currentTime <= element[1] + 0.5) {
        if (element[0] <= parseInt(currentTime) && element[2]) {
          element[2] = false;
          distressObserver(element[4], element[5]);
          console.log("start", event.currentTarget.currentTime);
        } else if (
          element[1] >= parseInt(currentTime) &&
          element[3] &&
          !element[2]
        ) {
          element[3] = false;
          distressNotObserver(element[4], element[5]);
          console.log("end", event.currentTarget.currentTime);
        }
      }
    });*/
    //     switch (parseInt(this.currentTime)) {
    //       case 8:
    //         distressObserver();
    //         break;
    //       case 16:
